...
mkdir -p my_build
cd my_build || exit
cmake -DCMAKE_BUILD_TYPE=Release -DENABLE_BUILD_LIB=ON -DENABLE_BLAS_LAPACK=OFF -DENABLE_GMP=OFF -DCMAKE_INSTALL_PREFIX=${INSTALL_DIR} -DCMAKE_OSX_SYSROOT="$OSX_SYSROOT" .. || exit
